...
function getImgName($img_url,$width=0,$height=0,$gen = 0)
{
	...
	if($width>0&&$height>0)
	{
		...
		$img_url_arr[0] = substr($img_url,0,-($len+1));
		$img_url_arr[1] = substr($img_url,-($len),$len);
...
function copyImage($origin_path,$sizes = array(),$path = 'share', $del_temp = true,$id = 0)
{
	...
	else
	{
		...
		if(!empty($data) && @file_put_contents($file_path,$data) > 0)
			$bln = true;
	 	*/
	 	$type=	getImageType($origin_path); //获取图片类型
...
//		}
		...
		if($type=='png'){
			...
			$data = getUrlContent($origin_path);  //获取远程图片
		}
